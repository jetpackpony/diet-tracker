...
import { Record, WeeklyRecordsFeedDocument, WeekRecords } from "../generated/graphql";
...
export const updateCachedTotals = (cache: ApolloCache<any>) => {
  const weeklyRecordsFeedQuery = cache.readQuery({ query: WeeklyRecordsFeedDocument, variables: { cursor: "" } });
...
export const removeRecordFromCache = (cache: ApolloCache<any>, recId: string) => {
  ...
  if (weeklyRecordsFeedQuery) {
    const newData = {
      weeklyRecordsFeed: {
        ...weeklyRecordsFeedQuery.weeklyRecordsFeed,
        weeks: updateTotals(deleteRecord(weeklyRecordsFeedQuery.weeklyRecordsFeed.weeks, recId))
      }
    };
    cache.writeQuery({
      query: WeeklyRecordsFeedDocument,
...
export const insertRecordIntoCache = (cache: ApolloCache<any>, newRecord: Record) => {
  const weeklyRecordsFeedQuery = cache.readQuery({ query: WeeklyRecordsFeedDocument, variables: { cursor: "" } });
  if (weeklyRecordsFeedQuery) {
    const newWeeks = weeklyRecordsFeedQuery.weeklyRecordsFeed.weeks.map((week) => {
            ...
            if (moment(newRecord.eatenAt).isBetween(day.dayStart, day.dayEnd)) {
              return {
                ...
                records: ([
                  ...
                  if (diff === 0) {
                    const diffCreate = moment(b.createdAt).diff(moment(a.createdAt));
                    return diffCreate;
                  } else {
                    return diff;
                  }
